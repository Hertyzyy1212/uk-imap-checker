...
with col1:
    ...
    st.caption("Upload a .txt file OR paste accounts below — email:password one per line")
    
    # 🆕 File Upload — right in the accounts section!
    ...
    # 📄 Combine file + pasted accounts
    all_accounts_text = ""
    if uploaded_file:
        file_content = uploaded_file.read().decode("utf-8", errors="ignore")
        all_accounts_text += file_content + "\n"
    if input_text.strip():
        all_accounts_text += input_text.strip()
...
# --- RUN CHECKS ---
if start_btn and all_accounts_text.strip():
    lines = [l.strip() for l in all_accounts_text.strip().split("\n") if l.strip()]
    accounts = []
    unknown_domains = []
    for line in lines:
        if ":" in line:
...
elif start_btn and not all_accounts_text.strip():
    st.warning("⚠️ Upload a file OR paste at least one account first!")
